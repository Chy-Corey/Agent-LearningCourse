...
```bash
uv add mcp anthropic python-dotenv openai
```
...
### DeepSeek API配置(参考：https://api-docs.deepseek.com/zh-cn/)
DEEPSEEK_API_KEY=
...
| 类名            | 角色          | 职责说明                                                     |
| --------------- | ------------- | ------------------------------------------------------------ |
| `Configuration` | 管理员        | 负责读取和验证环境变量（如 `DEEPSEEK_API_KEY`）              |
| `MCPServer`     | 工具管家      | 负责连接并管理本地的 MCP 服务器（例如天气脚本 `server.py`）。它知道有哪些工具可用，并负责让Server执行这些工具。 |
| `Tool`          | 工具说明书    | 将 MCP 工具的格式转换为 OpenAI/DeepSeek 兼容的格式，让大模型知道怎么调用这些工具。 |
| `MCPClient`     | 外交官/指挥官 | 核心业务逻辑。它连接 DeepSeek API，决定是直接回复用户，还是需要调用工具，并整合最终结果。 |

当你运行这个程序并输入一个问题（例如：“上海今天有暴雨预警吗？”）时，代码的执行流程如下：
    ...
    async def initialize(self) -> None:
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 使用 exit_stack 管理资源生命周期。stdio_client 会根据上面的参数
                # 在后台启动 server.py 子进程，并返回标准输入（write）和标准输出（stdio）的管道。
                stdio_transport = await self.exit_stack.enter_async_context(
                    stdio_client(server_params)
                )
                stdio, write = stdio_transport

                # 将输入输出管道交给 ClientSession，建立起客户端与服务端的双向通信会话。
...
注意：在 `client.py` 中已启用了 MCP Server，无需单独启用。