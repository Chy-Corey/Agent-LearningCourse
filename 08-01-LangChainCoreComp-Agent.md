# LangChain核心组件：智能体（Agent）

智能体将LLM与[Tools](https://langchain-doc.cn/v1/python/langchain/tools)结合，创建能够对任务进行推理、决定使用哪些工具并迭代寻求解决方案的系统。

在LangChain中，通过`create_agent` 方法可以将定义好的Model、Prompt、Tool、Memory等组件打包一起构建一个Agent。

[`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent) 方法提供了一个生产就绪的智能体实现。
智能体会一直循环运行，直到满足停止条件——即观察到模型输出最终结果或达到迭代次数限制。

<img src=".\image\08\mermaid-diagram.svg" alt="mermaid-diagram" style="width:25%;" />

## 一、核心组件

### 1. 模型Models

[Models](https://langchain-doc.cn/v1/python/langchain/models) 是智能体的推理引擎。它可以通过多种方式指定，支持静态和动态模型选择。
...
> **提示**
> 模型标识符字符串支持自动推断（例如 `"gpt-5"` 将被推断为 `"DeepSeek:gpt-5"`）。请参考以下链接查看完整的模型标识符字符串映射列表。
>
> [init_chat_model |langchain |LangChain 参考](https://reference.langchain.com/python/langchain/chat_models/base/init_chat_model)
>
> [Chat model integrations - Docs by LangChain](https://docs.langchain.com/oss/python/integrations/chat)

为了对模型配置进行更多控制，可以直接使用提供商包初始化模型实例。在此示例中，我们使用 `ChatDeepSeek`。请参阅 [聊天模型](https://langchain-doc.cn/v1/python/integrations/chat) 以了解其他可用的聊天模型类。
...
#### [动态模型](https://langchain-doc.cn/v1/python/langchain/agents.html#动态模型)

动态模型在运行时根据当前**状态**和**上下文**进行选择。这支持复杂的路由逻辑和成本优化。
...
- **`@wrap_model_call`**：这是一个装饰器，它的作用相当于在模型真正被调用之前，设置了一个“安检口”或“拦截器”。每次 Agent 想要调用大模型时，都会先经过这个函数。
...
advanced_model = ChatDeepSeek(model="deepseek-pro")	# 伪代码，目前并未支持26-05-21
...
> **警告**
> 在使用结构化输出时，不支持预绑定模型（已调用 [`bind_tools`](https://reference.langchain.com/python/langchain_core/language_models/#langchain_core.language_models.chat_models.BaseChatModel.bind_tools) 的模型）。如果需要使用结构化输出的动态模型选择，请确保传递给中间件的模型未预绑定。
...
当你使用 `@tool` 装饰器或者把普通函数传给 `create_agent` 时，LangChain 框架会在后台自动运行一套提取逻辑：

- **提取工具名称**：它会自动抓取你的函数名（比如 `search` 和 `get_weather`），作为工具的**名字**。
- **提取注释内容**：它会读取函数下方用 `"""` 包裹的文档字符串（docstring），也就是你写的“搜索信息”、“获取位置的天气信息”这些文字，作为工具的**描述**。
- **提取参数信息**：它还会通过 Python 的类型提示（比如 `query: str` 中的 `str`），分析出这个工具需要传入什么类型的参数。

这样，当把这些信息打包发给语言模型时，模型就能根据这些信息生成正确的返回输出。
...
假设用户输入导致 `get_weather` 工具内部发生了 `division by zero`（除零）错误：

1. **正常流程**：Agent 调用 `get_weather`。
2. **错误发生**：`get_weather` 执行出错，抛出异常。
3. **中间件拦截**：`handle_tool_errors` 捕获异常，不抛出，而是构造一个 `ToolMessage`。
4. **返回给 模型**：将错误告诉模型，让它重新调用工具。

#### ReAct 循环中的工具使用

LangChain中的Agent遵循 ReAct（“推理 + 行动”）模式，在简短的推理步骤与针对性的工具调用之间交替，并将结果观察反馈到后续决策中，直到能够提供最终答案。
...
#### [动态系统提示](https://langchain-doc.cn/v1/python/langchain/agents.html#动态系统提示)
...
可以通过向其 [`State`](https://langchain-doc.cn/v1/python/langgraph/graph-api#state) 传递更新来调用智能体。所有智能体在其状态中包含[消息序列](https://langchain-doc.cn/v1/python/langgraph/use-graph-api#messagesstate)；要调用智能体，请传递一条新消息：
...
#### [ProviderStrategy](https://langchain-doc.cn/v1/python/langchain/agents.html#providerstrategy)
...
### 2. [记忆](https://langchain-doc.cn/v1/python/langchain/agents.html#记忆)

智能体通过消息状态自动维护对话历史。您还可以配置智能体使用自定义状态模式，在对话期间记住额外信息。

**存储在状态中的信息**可以被视为智能体的[短期记忆](https://langchain-doc.cn/v1/python/langchain/short-term-memory)：
...
#### [通过中间件定义状态](https://langchain-doc.cn/v1/python/langchain/agents.html#通过中间件定义状态)
...
#### [通过 `state_schema` 定义状态](https://langchain-doc.cn/v1/python/langchain/agents.html#通过-state-schema-定义状态)
...
### 3. [流式传输](https://langchain-doc.cn/v1/python/langchain/agents.html#流式传输)
...
### 4. [中间件](https://langchain-doc.cn/v1/python/langchain/agents.html#中间件)
...
本章介绍了Agent以及其组件。可以理解的是，LangChain的Agent组件是一个集成部件，需要容纳其他的组件（如模型、Tools等）才能够完整使用。本章我们通过介绍Aget组件，也简单了解到了其他组件。

其他组件的具体使用方法将在后续详细介绍。

LangChain中的Agent概念和我们学习了解的Agent概念雷同：Agent是一个包含多个细分功能的结合体，而在LangChain中Agent组件就是一个需要集成多个组件的Home组件。